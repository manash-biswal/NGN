...
class TataSF004Two(Automation.Testcase):
    def __init__(self, tbFile, configFile, manifest, debug=True, displayName=None):
        super(TataSF004Two, self).__init__(displayName)
        self.tbFile = tbFile
				#Tata TBD Config File may need modification because of traffic_config
    ...
    def run(self):
        ...
        logger.debug('do_pings={}'.format(do_pings))
        ...
        # TATA TBD : check if any changes required in the config file for adding two CPEs -> looks like some config changes are required
...
"""
				#[Tata] : altered logs for this tc
        logger.debug('create_vpn={}, verify_vpn={}, create_subvpn={}, verify_subvpn={}, create_ad={}, tata_verify_ad={}'.format(create_vpn, verify_vpn, create_subvpn, verify_subvpn, create_ad, tata_verify_ad))
...
class TC_Dual_Cpe_Script(Automation.ScriptTemplate):
    def main(self):
        tc = TataSF004Two(
            ...
            displayName="XS Intra region communication"
        )
