...
"""
[Tata]: Delete Access-Design, assuming provisioned access-design, 1 vpn, 2 sub-VPN. Validate deletion of Access-design from CSO.
...
"""
class TataSF001Five(Automation.Testcase):

    def __init__(self,tbFile,configFile,manifest,debug=True,displayName=None):
        super(TataSF001Five,self).__init__(displayName)
        self.tbFile = tbFile
        ...
        self.subtests_list = [
            ...
            'subvpn_deletion', 'FUNC006 vpn deletion', 'tata ad verify','verify ad delete'
        ]
    ...
    def run(self):
        ...
        logger.debug('create_vpn={}, verify_vpn={}, create_ad={}, do_pings={}'.format(delete_ad, verify_ad_del))
        ...
        sub1 = self.configFile.configFile['service_chain']['subvpn_name1']
        sub2 = self.configFile.configFile['service_chain']['subvpn_name2']
        # [Tata]: sub3 = self.configFile.configFile['service_chain']['subvpn_name3']
        ...
        if create_vpn:
            ...
            try :
                ...
                try :
                    ...
                    sc = ts.gconn['cso'].create_service_chain(ts.gconn, self.dcs, "vpn_1sub.xml", self.vpn, sub=sub1, prefix = prefix)
                    if sc:
        ...
        cpeInfo = sc.fetch_dynamic_cpe_info(self.configFile.get_sub_config('create_cpe_config'), self.configFile.configFile)
        if not cpeInfo:
           ...
           return False
            ...
            for i in range(0,len(cpeInfo)):
                if 'subvpn_name1' in self.configFile.configFile['service_chain']:
                    # [Tata]: subvpn_count = self.configFile.configFile['service_chain']['subvpn_count']
                    #breakoutSubvpnCount = subvpn_count
                    #subvpn = self.configFile.configFile['service_chain']['subvpn_prefix']+str(count)
                    subvpn1 = self.configFile.configFile['service_chain']['subvpn_name1']
                    subvpn2 = self.configFile.configFile['service_chain']['subvpn_name2']
                else:
                ...
                # [Tata]: add 3 subvpn
                cpeInfo[i].update({'subvpn1':subvpn1})
                cpeInfo[i].update({'subvpn2':subvpn2})
                # [Tata]: if ('subvpn_count' in self.configFile.configFile['service_chain']) and (subvpn_count > 1):
                    #count += 1
        ...
        if create_ad:
            ...
            if cpeInfo:
                for i, site in enumerate(sites):
                    dc = cpeInfo[i]['dc']
                    template_xml = 'tata_add_1ad.xml'
                    logger.info("Adding site, using direct s end_netconf_rpc; %s cpe %s to vpn %s vrf %s, LAN %s, DC: %s, template_xml: %s" %
                    ...
                    if result:
                        count_ad_created += 1
                        logger.info('Access-design {} {} creation success. result={}, reply={}'.format(site['name'], site['sn'], result, reply))
                    else:
                        logger.error('Access-design {} {} creation failed. result={}, reply={}'.format(site['name'], site['sn'], result, reply))
        ...
        if verify_vpn:
            ...
            logger.info('Verifying service chain {},{}'.format(self.vpn, sub1))
            if sc.verify_service_chain_basic(sub1):
        ...
        # [Tata]: add very access-design block
        if tata_verify_ad:
        ...
        if do_pings:
            ...
            """
            self.subtest_start('Ping_CPE_internet')
            ...
            for cpe_sn in cpe_list:
                ...
                ping_result = sc.ping_cpe_internet(cpe_sn, sub1, '8.8.8.8')
                if ping_result:
            ...
            ping_result = sc.ping_cpe_cpe(cpe_list, sub1)
            if ping_result:
            ...
            lisp_cnt_result = sc.verify_lisp_counter(cpe_list, sub1)
            if lisp_cnt_result:
        ...
        if delete_ad:
            ...
            try:
                ...
                if result:
                    ...
                    self.subtest_fail('access_design_deletion', 'Failed to delete AD.')
            except:
                logger.exception("Failed to delete access-design on %s" % self.vpn)
                self.set_test_failed()
                # [Tata]: Modified to delete one access-design
            """ [Tata]: count_ad_deleted = 0
                for i, site in enumerate(sites):
        ...
        # [Tata]: Added code to verify deletion of access design
        if verify_ad_del:
            self.subtest_start('verify ad delete')
            ad = 0
            try:
                # [Tata]: Added get_access_design_list method in NCS.py
                while site['name'] in ts.gconn['cso_cli'].get_access_design_list():
                    logger.info("access-design present")
                    ad = 1
                if ad == 0:
                    self.subtest_pass('verify ad delete')
                else:
                    self.subtest_fail('verify ad delete', "ad not deleted")
            except:
                logger.exception("connection not initialized.. return")
                self.subtest_fail('verify ad delete', 'connection to cso not initialized')
                return
        ...
        if delete_sub:
            ...
            logger.info("Deleting individual subvpn %s" % sub1)
            template_xml = 'ngena_delete_subvpn.xml'
            params = { 'vpn': self.vpn , 'sub': sub1 }
            logger.info('Deleting subvpn {}'.format(params))
            result, reply = ts.gconn['cso'].send_netconf_rpc(template_xml, params)
            if result is True:
                self.subtest_pass('subvpn_deletion')
                logger.info("subvpn %s deleted successfully" % sub1)
            else:
                self.subtest_fail('subvpn_deletion', failure=reply)
                logger.error("subvpn %s deletion failed" % sub1)
...
"""
class TataSF001Five(Automation.ScriptTemplate):
