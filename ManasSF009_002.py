...
class TC_ASA_Add_Rule(Automation.Testcase):
    ...
    def run(self):
        ...
        template_xml = self.configFile['add_rule_permit_any_any']
        params = {
            'vpn': vpn,
            'sub': sub,
        }
        ...
        if not ret[0]:
            self.subtest_fail('Configure_Validate_ASA_Rules', 'Configuration of rules failed')
        ...
        try:
            ...
            if not ret:
                self.subtest_fail('Unable to add ASA rule', 'Rule validations on CSO and ASA failed')


        except:
            logger.exception('failed to add ASA rule')
            self.subtest_fail('Unable to add ASA rule', 'Exception while rule change in ASA')
        self.set_test_passing()

"""
TC_ASA_Add_Rule Script derived class definition,
derived from class ScriptTemplate in lib/Automation.py to be used for this unit test
"""
class TC_ASA_Add_Rule_Script(Automation.ScriptTemplate):
    ...
    def main(self):
        tc = TC_ASA_Add_Rule(
            ...
            displayName="I am running TC_ASA_Add_Rule"
        )
...
if __name__ == "__main__":
    time.sleep(4)
    script = TC_ASA_Add_Rule_Script(loadConfigFile=True)
    script.main()
