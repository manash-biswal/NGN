...
"""
[Tata]: Provision single sub-VPN assuming already present Vpn. Validate sub-VPN provisioning.
...
class TataSF001Two(Automation.Testcase):
    def __init__(self, tbFile, configFile, manifest, debug=True, displayName=None):
        super(TataSF001Two, self).__init__(displayName)
        self.tbFile = tbFile
    ...
    def run(self):
        ...
        logger.debug('create_subvpn={}, verify_subvpn={}'.format(create_subvpn, verify_subvpn))
...
class TataSF001Two(Automation.ScriptTemplate):
    def main(self):
        tc = TataSF001Two(
            self.tbFile,
            self.manifest,
