...
"""
[Tata]: Delete all the sub-VPN assuming 1VPN, 1 sub-VPN present. Validate deletion of all sub-VPN
...
class TataSF001Eight(Automation.Testcase):
    def __init__(self, tbFile, configFile, manifest, debug=True, displayName=None):
        super(TataSF001Eight, self).__init__(displayName)
        self.tbFile = tbFile
        ...
        self.subtests_list = [
            ...
            'all_subvpn_deletion', 'FUNC006 vpn deletion', 'tata vpn verify', 'tata subvpn verify',
            'tata del all subvpn verify'
        ]
    ...
    def run(self):
        ...
        if delete_all_sub:
            ...
            if result is True:
                self.subtest_pass('all_subvpn_deletion')
                logger.info("subvpns container %s deleted successfully" % self.vpn)
            else:
                self.subtest_fail('all_subvpn_deletion', failure=reply)
                logger.error("subvpns container %s deletion failed" % self.vpn)


        if verify_del_all_subvpn:
            self.subtest_start('tata del all subvpn verify')
            subvpn=0
            try:
                while sub in ts.gconn['cso_cli'].get_subvpn_list():
                    logger.info("subVPN created successfully %s" % self.vpn)
                    subvpn=1 # [Tata]: Replace the number of sub-vpn here with number present.
                if subvpn == 0:
                    self.subtest_pass('tata del all subvpn verify')
                else:
                    self.subtest_fail('tata del all subvpn verify',"subvpn not deleted")
            except:
                logger.exception("connection not initialized.. return")
                self.subtest_fail('tata del all subvpn verify', 'connection to cso not initialized')
                return
...
class TataSF001Eight(Automation.ScriptTemplate):
    def main(self):
        tc = TataSF001Eight(
            self.tbFile,
            self.manifest,
            debug=self.options.debug,
            displayName="Delete Single vpn multiple subvpn operations"
        )
