...
"""
[Tata]: Delete VPN assuming presence of 1 VPN. Validate deletion of VPN from CSO.
...
class TataSF001Nine(Automation.Testcase):
    def __init__(self, tbFile, configFile, manifest, debug=True, displayName=None):
        super(TataSF001Nine, self).__init__(displayName)
        self.tbFile = tbFile
    ...
    def run(self):
        ...
        delete_sub = False
        delete_vpn = True
        verify_vpn_delete = True

        # [Tata]: Hashed subVpn subtest start
        logger.debug('create_vpn={}, verify_vpn={}, create_ad={}, do_pings={}'.format(create_vpn, verify_vpn, create_ad,
                                                                                      do_pings))
        ...
        if create_vpn:
            ...
            try:
                ...
                try:
                    ...
                    # [tata]: removed subvpn and prefix params from create_service_chain, changed xml template
        ...
        # [tata]: Added code block to verify delete VPN.
        if verify_vpn_delete:
            self.subtest_start('tata delete vpn verify')
            vpn=0
            try:
                while self.vpn in ts.gconn['cso_cli'].get_vpn_list():
                    logger.info("VPN created successfully %s" % self.vpn)
                    vpn=1
                if vpn == 0:
                    self.subtest_pass('tata delete vpn verify')
                else:
                    self.subtest_fail('tata delete vpn verify', "VPN not created")
            except:
                logger.exception("connection not initialized.. return")
                self.subtest_fail('tata delete vpn verify', 'connection to cso not initialized')
                return
...
class TataSF001Nine(Automation.ScriptTemplate):
    def main(self):
        tc = TataSF001Nine(
            self.tbFile,
            self.manifest,
            debug=self.options.debug,
            displayName="Delete vpn operations"
        )
