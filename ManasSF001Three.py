...
"""
[Tata]: Provision two sub-VPN assuming already present Vpn, one sub-VPN. Validate multiple sub-VPN provisioning.
...
class TataSF001Three(Automation.Testcase):
    def __init__(self, tbFile, configFile, manifest, debug=True, displayName=None):
        super(TataSF001Three, self).__init__(displayName)
        self.tbFile = tbFile
        ...
        self.subtests_list = [
            'pre-test',
            'FUNC001_vpn_creation', 'FUNC002_subvpn_creation','multi-subvpn-creation'
    ...
    def run(self):
        ...
        create_subvpn_multi = True
        create_subvpn = False
        Create_vpn = False
        create_ad = False
        verify_vpn = True
        verify_multisubvpn = True
        ...
        logger.debug('create_subvpn={}, verify_vpn={}, create_ad={}, do_pings={}'.format(create_subvpn_multi, verify_vpn, verify_multisubvpn))
        # [Tata]: self.subtest_start('FUNC001_vpn_creation')
        self.subtest_start('multi_subvpn_creation')
        ...
        if create_subvpn_multi:
            ...
            try:
                ...
                try:
                    prefix1 = self.configFile.configFile['service_chain']['prefix_nw1']
                    prefix2 = self.configFile.configFile['service_chain']['prefix_nw2']
                    # [Tata]: sc = ts.gconn['cso'].create_service_chain(ts.gconn, self.dcs, "vpn_2sub.xml", self.vpn, sub=sub, prefix = prefix)
                    # [Tata]: getting send_netconf_rpc method out of create_service_chain method to support multiple sub-vpn
                    params = {
                        'msgID': self.msgID,
                        'vpn': self.vpn,
                        'sub1': sub1,
                    ...
                    result, reply = ts.gconn['cso'].send_netconf_rpc("vpn_2sub.xml", params)
                    if result:
                    ...
                    if sc:
                        ...
                        self.subtest_fail('multi_subvpn_creation')
                        logger.error("multi subvpn creation request failed")
                        return
                    ...
                    self.subtest_fail('multi_subvpn_creation')
                    logger.error("Exception!! Multi subvpn creation request failed")
                    return
                ...
                try:
                    ...
                    self.subtest_failed('multi_subvpn_creation',
                    ...
                    self.subtest_failed('multi_subvpn_creation', 'Unexpected error')
                    # [tata]: self.subtest_failed('FUNC002_subvpn_creation')
                    return
                else:
                    logger.info("vpn infra ICSP %s is now ready" % self.vpn)
                    # [tata]: self.subtest_pass('FUNC001_vpn_creation')
                    self.subtest_pass('multi_subvpn_creation')
        ...
        # [tata]: Added code block to verify multiple sub-VPN.
        if verify_multisubvpn:
            self.subtest_start('tata multisubvpn verify')
            multi=0
            try:
                while sub1 in ts.gconn['cso_cli'].get_subvpn_list():
                    logger.info("subVPN1 created successfully")
                    multi = multi + 1;
                while sub2 in ts.gconn['cso_cli'].get_subvpn_list():
                    logger.info("subVPN2 created successfully")
                    multi = multi + 1;
                if multi == 2:
                    self.subtest_pass('tata multisubvpn verify')
                else:
                    self.subtest_fail('tata multisubvpn verify')
            except:
                logger.exception("connection not initialized.. return")
...
class TataSF001Three(Automation.ScriptTemplate):
    def main(self):
        tc = TataSF001Three(
            self.tbFile,
            self.manifest,
            debug=self.options.debug,
            displayName="Single vpn Multi subvpn operations"
        )
