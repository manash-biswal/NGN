...
"""
TC_ASA_CheckSequence derived class definition,
derived from class Testcase in lib/Automation.py to be used for this unit test
"""
class TC_ASA_CheckSequence(Automation.Testcase):

    def __init__(self,tbFile,configFile,debug=True,displayName=None):
        super(TC_ASA_CheckSequence,self).__init__(displayName)
        self.tbFile = tbFile
    ...
    def run(self):
        ...
        try:
            ...
            if not ret:
                self.subtest_fail('Configure_Validate_ASA_Rules', 'Rule validations on CSO and ASA failed')

        except:
            logger.exception('failed to validate ASA rules')
            self.subtest_fail('Configure_Validate_ASA_Rules', 'Exception while rule validations ASA')
        self.set_test_passing()

"""
TC_ASA_CheckSequence_Script derived class definition,
derived from class ScriptTemplate in lib/Automation.py to be used for this unit test
"""
class TC_ASA_CheckSequence_Script(Automation.ScriptTemplate):

    def main(self):
        tc = TC_ASA_CheckSequence(
            self.tbFile,
            self.configFile,
            debug=self.options.debug,
            displayName="I am running TC_ASA_CheckSequence_Script"
        )
...
if __name__ == "__main__":
    time.sleep(4)
    script = TC_ASA_CheckSequence_Script(loadConfigFile=True)
    script.main()
