...
"""
[Tata]: Delete all Access-design present. Validate deletion of all access-design.
...
"""
class TataSF001Seven(Automation.Testcase):

    def __init__(self,tbFile,configFile,manifest,debug=True,displayName=None):
        super(TataSF001Seven,self).__init__(displayName)
        self.tbFile = tbFile
        ...
        self.subtests_list = [
            ...
            'all_access_design_deletion', 'CPE_offboarding',
            'subvpn_deletion', 'FUNC006 vpn deletion', 'tata ad verify','verify all ad delete'
        ]
    ...
    def run(self):
        ...
        verify_all_ad_del=True
        do_pings = False
        delete_all_ad = True
        delete_sub = False
        delete_vpn = False

        logger.debug('create_vpn={}, verify_vpn={}, create_ad={}, do_pings={}'.format(create_vpn, verify_vpn, create_ad, do_pings))
        ...
        if create_ad:
            ...
            if cpeInfo:
                for i, site in enumerate(sites):
                    ...
                    logger.info("Adding site, using direct send_netconf_rpc; %s cpe %s to vpn %s vrf %s, LAN %s, DC: %s, template_xml: %s" %
        ...
        if delete_all_ad:
            self.subtest_start('all_access_design_deletion')
            try:
        ...
        if verify_all_ad_del:
            self.subtest_start('verify all ad delete')
            ad = 0
            try:
                # [Tata]: Added get_access_design_list method in NCS.py
                while site['name'] in ts.gconn['cso_cli'].get_access_design_list():
                    logger.info("access-design present")
                    ad = 1 # [Tata]: replace with number of access designs present
                if ad == 0:
                    self.subtest_pass('verify all ad delete')
                else:
                    self.subtest_fail('verify all ad delete', "ad not deleted")
            except:
                logger.exception("connection not initialized.. return")
                self.subtest_fail('verify all ad delete', 'connection to cso not initialized')
                return
...
"""
class TataSF001Seven(Automation.ScriptTemplate):
    ...
    def main(self):
        tc = TataSF001Seven(
            ...
            displayName="All access-design delete operations"
        )
